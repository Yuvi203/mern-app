...
const sendEmailRegsister = (to, url, text)=>{
...
smtTransport.sendMail(mailOptions, (err, info)=>{
 if (err) return {err};
 return info
})
}
...
const sendEmail = (name, email, subject, message, user_mailid) =>{
  oauth2client.setCredentials({
   refresh_token:G_REFRESH_TOKEN
  })
  const smtpTransport  = nodemailer.createTransport({
...
module.exports = {sendEmailRegsister, sendEmailReset, sendEmail}